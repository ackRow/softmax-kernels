...
torch::Tensor launch_softmax_cuda_multi_block_v1(torch::Tensor in)
{
...
torch::Tensor softmax_cuda_multi_block_v1(torch::Tensor in) {
    ...
    if (w >= 16384)
        return launch_softmax_cuda_multi_block_v1<32>(in);
    else if (w >= 4096)
        return launch_softmax_cuda_multi_block_v1<16>(in);
    else if (w >= 2048)
        return launch_softmax_cuda_multi_block_v1<8>(in);
    else
        return launch_softmax_cuda_multi_block_v1<4>(in);
}
