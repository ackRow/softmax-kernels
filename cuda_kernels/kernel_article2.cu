...
template <typename T, int num_warps, int n_cols>
__global__ void
__launch_bounds__(512, 4)
softmax_kernel_multi_block_v2(T* __restrict__ input, T* __restrict__ output, int n_rows) {
    constexpr int block_size = num_warps * WARP_SIZE;
    constexpr int scaled_width = n_cols / 4;
    constexpr int unroll_factor = CEIL_DIV(scaled_width, block_size);
    ...
    for (int row_index = blockIdx.x*blockDim.x + threadIdx.x; row_index < n_rows; row_index += gridDim.x) {
        ...
        for (int i = 0; i < scaled_width; i += block_size) {
            max_val = fmaxf(max_val, max_float4(in_vector[thread_index + i]));
        }
        max_val = block_wide_max(num_warps, thread_index, max_val, reduction);
        ...
        #pragma unroll unroll_factor
        for (int i = 0; i < scaled_width; i += block_size) {
            denominator += sum_float4(stable_exp_float4(in_vector[thread_index + i], max_val));
        }
        denominator = block_wide_sum(num_warps, thread_index, denominator, reduction);
        ...
        #pragma unroll unroll_factor
        for (int i = 0; i < scaled_width; i += block_size) {   
            auto numerator = stable_exp_float4(in_vector[thread_index + i], max_val);
            out_vector[thread_index + i] = divide_float4(numerator, denominator);
        }
    }
}

template <int num_warps, int n_cols>
torch::Tensor launch_softmax_cuda_multi_block_v2(torch::Tensor in)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(in.scalar_type(), "softmax_kernel_multi_block_v2", ([&] {
        softmax_kernel_multi_block_v2<scalar_t, num_warps, n_cols><<<grid_size, block_size>>>(
            in.data_ptr<scalar_t>(), out.data_ptr<scalar_t>(), n_rows
        );
    }));
